...
import { MessageSquare, Heart, Share2, CornerDownRight, Filter } from 'lucide-react';
...
export const CommentFeed: React.FC<CommentFeedProps> = ({ comments, currentTime }) => {
  ...
  // Filter comments that have unlocked based on simulation timeline
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 max-h-[440px] overflow-y-auto pr-1">
        {filteredComments.length === 0 ? (
          <div className="py-12 text-center border border-dashed border-white/10 rounded-2xl p-4 bg-zinc-950/40">
            <MessageSquare className="w-6 h-6 text-zinc-600 mx-auto mb-2" />
            <p className="text-xs text-zinc-400 font-medium">
              No simulated comments unlocked yet.
            </p>
            <span className="text-[10px] text-zinc-500">
              Run simulation forward to generate audience reactions.
            </span>
          </div>
        ) : (
          filteredComments.map((comment) => (
            <div
              key={comment.id}
              className="p-3.5 rounded-2xl bg-zinc-950/70 border border-white/10 space-y-2 hover:border-indigo-500/30 transition-all"
            >
              {/* Comment Header */}
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2.5">
                  <div
                    ...
                    style={{ backgroundColor: `${comment.authorColor}25` }}
                  >
              ...
              <div className="flex items-center space-x-4 pl-9 text-[10px] text-zinc-400 pt-1 font-mono">
                ...
                <span className="flex items-center space-x-1 hover:text-indigo-400 transition-colors">
